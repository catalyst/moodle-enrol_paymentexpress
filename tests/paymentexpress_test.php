...
/**
 * paymentexpress enrolment plugin tests.
 *
...
class enrol_paymentexpress_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['paymentexpress'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['paymentexpress']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('paymentexpress'));
        $plugin = enrol_get_plugin('paymentexpress');
        $this->assertInstanceOf('enrol_paymentexpress_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_paymentexpress', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $paymentexpressplugin = enrol_get_plugin('paymentexpress');

        // Just make sure the sync does not throw any errors when nothing to do.
        $paymentexpressplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_paymentexpress_plugin $paymentexpressplugin  */
        $paymentexpressplugin = enrol_get_plugin('paymentexpress');
        ...
        $id = $paymentexpressplugin->add_instance($course1, $data);
        $instance1  = $DB->get_record('enrol', array('id' => $id));
        $data = array('roleid' => $studentrole->id, 'courseid' => $course2->id);
        $id = $paymentexpressplugin->add_instance($course2, $data);
        ...
        $paymentexpressplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $paymentexpressplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $paymentexpressplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now - 60);

        $paymentexpressplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $paymentexpressplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now - 60 * 60);
        $paymentexpressplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60 * 60);

        $paymentexpressplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now - 60 * 60 * 24 * 7, $now - 60);
        $paymentexpressplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $paymentexpressplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $paymentexpressplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));

        $paymentexpressplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $paymentexpressplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $paymentexpressplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        $pluginname = 'paymentexpress';

        // Only enable the paymentexpress enrol plugin.
        ...
        // Payment Express enrolment has 2 enrol actions for active users when logged in as admin: edit and unenrol.
        ...
        $actions = $plugin->get_user_enrolment_actions($manager, $ue);
        // Teachers don't have the enrol/paymentexpress:unenrol capability by default, but have enrol/paymentexpress:manage.
